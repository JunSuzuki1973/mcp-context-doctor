...
接続数だけで危険度を判定せず、「全ツール定義を読み込む場合の推定トークン数」を表示します。遅延ロード、会話履歴、ツール出力、組み込み指示は別の要因なので、実際の残りコンテキストやオーバーフロー確率とは表現しません。
...
全件ロード推定と、出力スキーマ等を含むカタログ計測値は重複するため合算しません。tiktokenの値はClaude独自の正確なトークン数ではありません。失敗した接続は「未計測」でありゼロ扱いしません。
