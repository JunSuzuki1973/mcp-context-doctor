...
```sh
uv tool install "git+https://github.com/JunSuzuki1973/mcp-context-doctor.git@v0.2.1"
```
...
ホストが全定義を先読みするか、使う時に取得するかで注目すべき数字が変わりますが、**カタログにはその手がかりがありません**。したがって `--loading deferred|eager` で利用者が宣言します（推測しません）。既定の `unknown` は下限・上限の両方を出し、どちらも断定しません。

ツール出力は測定しません。今後も測定しません — 収集側は discovery のみを行い `tools/call` を発行しないため、数えるべき応答が存在しないからです。`runtime_output_tokens` は常に `null` です。代わりに、カタログと設定が**宣言している**内容を `output_bound` として記録します（入力スキーマ中の件数制限パラメータの有無、設定側の出力トークン上限の有無）。`outputSchema` は形を定めるだけでサイズを制限しないため、**上限としては数えません**。
